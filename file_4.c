 ...
 * @output_buffer: output_buffer array to handle print
	...
	int flags, int width, int precision, int size)
{
	...
	return (handle_write_char(f, output_buffer, flags, width, precision, size));
}
	...
	int flags, int width, int precision, int size)
{
	...
	while (number > 0)
	{
		output_buffer[l--] = (number % 10) + '0';
		number /= 10;
	}

	l++;

	return (write_number(is_negative, l, output_buffer, flags, width, precision, size));
}
